...
//All Strings
String welcomeMessage = "GRP16 BE PROJECT";
...
String invalidMessage = "USE A VALID CARD/TAG FOR CHARGING";
  ...
  if (rfid.PICC_IsNewCardPresent()) { // new tag is available
    if (rfid.PICC_ReadCardSerial()) { // NUID has been readed
      ...
      if (rfid.uid.uidByte[0] == authorizedUID[0] &&
        ...
        lcd.setCursor(0,0);
        lcd.print(welcomeMessage);
        scrollMessage(1, invalidMessage, 250, totalColumns);
       ...
       if(tap == 0){
        ...
        Serial.println(tap);
          ...
          rfid.uid.uidByte[1] == authorizedUID[1] &&
          rfid.uid.uidByte[2] == authorizedUID[2] &&
          rfid.uid.uidByte[3] == authorizedUID[3] && tap == 1){
        digitalWrite(RELAY_PIN, HIGH);  // deactivate the relay
        tap = 0;
        Serial.println(tap);
        }
